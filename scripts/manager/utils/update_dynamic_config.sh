...
update_dynamic_config(){
  ...
  reset_dynamic_config(){
    ...
    if rcon_execute_commands commands[@]; then
      log "[SUCCESS] EXÉCUTION DE LA COMMANDE RCON: ForceUpdateDynamicConfig RÉUSSIE SUR LE SERVEUR: $SERVER_SERVICE_NAME."
    else
      log "[ERROR] ERREUR LORS DE L'EXÉCUTION DE LA COMMANDE RCON: ForceUpdateDynamicConfig SUR LE SERVEUR: $SERVER_SERVICE_NAME."
      log "[DEBUG] VÉRIFIEZ QUE LE SERVEUR: $SERVER_SERVICE_NAME EST BIEN DÉMARRÉ."
      return
    fi
  }
  ...
  compare_dynamic_config(){
    local day="$1"
    log "[LOG] COMPARAISON DES FICHIERS DE CONFIGURATION DYNAMIQUE ACTUEL ET DU JOUR: $day SUR LE SERVEUR: $SERVER_SERVICE_NAME EN COURS..."
