...
install_rcon_cli(){
  ...
  log "[LOG] VÉRIFICATION DE L'INSTALLATION DE RCON CLI POUR LE SERVEUR ARK: SURVIVAL ASCENDED..."
  if ! [[ -f "$RCON_EXECUTABLE_PATH" ]]; then
    log "[WARNING] RCON CLI N'EST PAS INSTALLÉ, INSTALLATION EN COURS..."
    ...
    if cd "$RCON_TEMP_PATH"; then
      ...
      log "[DEBUG] VEUILLEZ ESSAYER DE VOUS DÉPLACER MANUELLEMENT DANS LE RÉPERTOIRE AVEC LA COMMANDE: cd $RCON_TEMP_PATH"
      exit 1
    fi
    ...
    log "[LOG] CONFIGURATION DES PERMISSIONS D'EXÉCUTION POUR L'UTILISATEUR $USER_ACCOUNT SUR LE BINAIRE RCON CLI..."
    if chmod +x "$RCON_EXECUTABLE_PATH"; then
      log "[SUCCESS] PERMISSIONS D'EXÉCUTION POUR LE BINAIRE RCON CLI CONFIGURÉES AVEC SUCCÈS POUR L'UTILISATEUR $USER_ACCOUNT."
      ...
      log "[DEBUG] VEUILLEZ ESSAYER DE CONFIGURÉ LES PERMISSIONS MANUELLEMENT AVEC LA COMMANDE: chmod +x $RCON_EXECUTABLE_PATH"
      exit 1
    fi

  else
    log "[OK] RCON CLI EST CORRECTEMENT INSTALLÉ DANS LE RÉPERTOIRE $RCON_PATH POUR L'UTILISATEUR $USER_ACCOUNT."
    ls -l "$RCON_EXECUTABLE_PATH"
  fi
