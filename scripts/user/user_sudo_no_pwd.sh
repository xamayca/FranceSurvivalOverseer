...
user_sudo_no_pwd(){
  ...
  if sudo grep -R "^$USER_ACCOUNT .*NOPASSWD:ALL" /etc/sudoers.d &>/dev/null; then
    ...
    if echo "$USER_ACCOUNT ALL=(ALL) NOPASSWD:ALL" | sudo EDITOR='tee' visudo -f /etc/sudoers.d/"$USER_ACCOUNT"; then
      log "[SUCCESS] L'UTILISATEUR $USER_ACCOUNT PEUT MAINTENANT ÉXÉCUTER DES COMMANDES SUDO SANS MOT DE PASSE."
    else
      log "[ERROR] UNE ERREUR S'EST PRODUITE LORS DE L'AJOUT DE L'UTILISATEUR $USER_ACCOUNT À LA LISTE DES UTILISATEURS SANS MOT DE PASSE."
