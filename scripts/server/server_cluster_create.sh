...
server_cluster_create() {
  ...
  mount_nfs_cluster(){
    ...
    if ping -c 1 -W 1 "$NFS_IP_ADDRESS" &>/dev/null; then
      log "[SUCCESS] LE SERVEUR ARK: $SERVER_SERVICE_NAME COMMUNIQUE AVEC LE NFS A L'ADRESSE IP: $NFS_IP_ADDRESS."
    else
    ...
    log "[LOG] VÉRIFICATION DE L'EXISTENCE DU RÉPERTOIRE DE CLUSTER NFS SUR LE SERVEUR ARK: $SERVER_SERVICE_NAME..."
    if ! [[ -d "$CLUSTER_DIR_OVERRIDE" ]]; then
      ...
      log "[OK] LE RÉPERTOIRE DE CLUSTER NFS EXISTE DÉJÀ SUR LE SERVEUR ARK: $SERVER_SERVICE_NAME."
    fi
  ...
  log "[LOG] VÉRIFICATION DU MONTAGE DU CLUSTER NFS SUR $HOSTNAME POUR LE SERVEUR ARK: $SERVER_SERVICE_NAME..."
  if mount | grep -q "$NFS_IP_ADDRESS:$NFS_FOLDER_PATH on $CLUSTER_DIR_OVERRIDE type nfs"; then
    log "[OK] LE CLUSTER NFS EST MONTÉ ET CONFIGURÉ CORRECTEMENT SUR $HOSTNAME."
  else
    log "[WARNING] LE CLUSTER NFS N'EST PAS MONTÉ OU CONFIGURÉ CORRECTEMENT."
    log "[INFO] VÉRIFICATION DES VARIABLES DE CONFIGURATION..."
    check_variables
    ask_for_install
  fi
