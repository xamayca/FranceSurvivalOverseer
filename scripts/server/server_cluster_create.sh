...
server_cluster_create() {
  ...
  check_variables(){
    if [ -z "$MULTIHOME" ]; then
      ...
      exit 1
    elif [ -z "$NAS_IP_ADDRESS" ]; then
      log "[ERROR] VEUILLEZ DÉFINIR L'ADRESSE IP DU NAS POUR LE CLUSTER NFS DANS LA VARIABLE NAS_IP_ADDRESS DU FICHIER DE CONFIGURATION."
      log "[INFO] EXEMPLE D'ADRESSE IP DU NAS: 192.168.1.XX"
      exit 1
    elif [ -z "$NAS_FOLDER_PATH" ]; then
      log "[ERROR] VEUILLEZ DÉFINIR LE CHEMIN DU DOSSIER SUR LE NAS POUR LE CLUSTER NFS DANS LA VARIABLE NAS_FOLDER_PATH DU FICHIER DE CONFIGURATION."
      log "[INFO] EXEMPLE DE CHEMIN DU DOSSIER SUR LE NAS: /volume1/CLUSTER"
      exit 1
    fi
  }
  ...
  ask_for_install(){
    ...
    log "[ATTENTION] VÉRIFIER QUE LE RÉPERTOIRE PARTAGÉ SUR VOTRE NAS EST CRÉÉ ET CONFIGURÉ CORRECTEMENT."
    log "[ATTENTION] VÉRIFIER LES AUTORISATIONS D'ACCÈS AU RÉPERTOIRE PARTAGÉ SUR VOTRE NAS VERS LE SERVEUR ARK."
    log "[ATTENTION] VÉRIFIER QUE LE PARE-FEU DE VOTRE NAS AUTORISE L'ACCÈS AU RÉPERTOIRE PARTAGÉ PAR LE SERVEUR ARK"
    log "[ATTENTION] VÉRIFIER QUE LES PORTS NFS SONT OUVERTS AUTORISÉS DANS LE PARE-FEU DE VOTRE NAS."
  ...
  mount_nfs_cluster(){
    ...
    log "[LOG] VÉRIFICATION & CRÉATION DE LA CONNEXION AU NAS POUR LE CLUSTER NFS SUR LE SERVEUR ARK: $SERVICE_NAME..."

    if ping -c 1 -W 1 "$NAS_IP_ADDRESS" &>/dev/null; then
      log "[SUCCESS] LE SERVEUR ARK: $SERVICE_NAME COMMUNIQUE AVEC LE NAS A L'ADRESSE IP: $NAS_IP_ADDRESS."
    else
      log "[ERROR] LE SERVEUR ARK: SURVIVAL ASCENDED NE COMMUNIQUE PAS AVEC LE NAS."
      log "[DEBUG] VEUILLEZ VÉRIFIER L'ADRESSE IP DU NAS DANS LE FICHIER DE CONFIGURATION."
      exit 1
    fi
    ...
    if ! mount | grep -q "$NAS_IP_ADDRESS:$NAS_FOLDER_PATH on $CLUSTER_DIR_OVERRIDE type nfs"; then
      log "[WARNING] LE MONTAGE NFS POUR LE CLUSTER NFS N'EXISTE PAS SUR $HOSTNAME, MONTAGE EN COURS..."
      if sudo mount -t nfs -o vers=3,rw,hard "$NAS_IP_ADDRESS:$NAS_FOLDER_PATH" "$CLUSTER_DIR_OVERRIDE"; then
        log "[SUCCESS] LE MONTAGE NFS POUR LE CLUSTER NFS A ÉTÉ MONTÉ AVEC SUCCÈS SUR $HOSTNAME."
      else
        log "[ERROR] UNE ERREUR S'EST PRODUITE LORS DU MONTAGE NFS POUR LE CLUSTER NFS SUR $HOSTNAME."
        log "[DEBUG] VEUILLEZ MONTÉ LE CLUSTER NFS À L'AIDE DE LA COMMANDE SUIVANTE:"
        log "[DEBUG] sudo mount -t nfs -o vers=3,rw,hard $NAS_IP_ADDRESS:$NAS_FOLDER_PATH $CLUSTER_DIR_OVERRIDE"
        exit 1
      fi
    else
    ...
    if ! grep -q "$NAS_IP_ADDRESS:$NAS_FOLDER_PATH $CLUSTER_DIR_OVERRIDE nfs" /etc/fstab; then
      log "[WARNING] L'AUTO MONTAGE DU CLUSTER NFS N'EST PAS CONFIGURÉ DANS /etc/fstab, CONFIGURATION EN COURS..."
      if echo "$NAS_IP_ADDRESS:$NAS_FOLDER_PATH $CLUSTER_DIR_OVERRIDE nfs defaults 0 0" | sudo tee -a /etc/fstab; then
        ...
        log "[DEBUG] echo $NAS_IP_ADDRESS:$NAS_FOLDER_PATH $CLUSTER_DIR_OVERRIDE nfs defaults 0 0 | sudo tee -a /etc/fstab"
        exit 1
      fi
    else
    ...
    if mount | grep -q "$NAS_IP_ADDRESS:$NAS_FOLDER_PATH on $CLUSTER_DIR_OVERRIDE type nfs"; then
      ...
      log "[DEBUG] mount | grep $NAS_IP_ADDRESS:$NAS_FOLDER_PATH on $CLUSTER_DIR_OVERRIDE type nfs"
      exit 1
    fi
