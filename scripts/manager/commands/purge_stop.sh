...
purge_stop(){
  ...
  log "[WARNING] DÉSACTIVATION DE LA PURGE PVP SUR LE SERVEUR ARK $SERVER_SERVICE_NAME SUR $HOSTNAME EN COURS..."
  ...
  if grep -q "PreventOfflinePvP=True" "$ARK_SERVICE_PATH"; then
    log "[OK] LA PURGE EST DÉJÀ DÉSACTIVÉE SUR LE SERVEUR ARK $SERVER_SERVICE_NAME."
    return 0
  else
    update_command_line "edit_params" "PreventOfflinePvP" "True"
  fi
  ...
  local messages=()
  for i in {1..5}; do
    message_var="RCON_PURGE_STOP_MSG_$i"
  ...
  log "[LOG] REDÉMARRAGE DU SERVICE $SERVER_SERVICE_NAME POUR DESACTIVATION DE LA PURGE PVP..."
