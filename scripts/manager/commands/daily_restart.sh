...
daily_restart() {
  ...
  log "[WARNING] REDÉMARRAGE JOURNALIER DU SERVEUR ARK: $SERVER_SERVICE_NAME SUR $HOSTNAME EN COURS..."
  log "[LOG] VÉRIFICATION DE LA VALEUR DE Restart DANS LE FICHIER DE CONFIGURATION DU SERVICE $SERVER_SERVICE_NAME..."
  ...
  local messages=()
    for i in {1..5}; do
      message_var="RCON_DAILY_RESTART_$i"
  ...
  if sudo systemctl restart "$SERVER_SERVICE_NAME"; then
    ...
    log "[DEBUG] VEUILLEZ ESSAYER DE REDÉMARRER LE SERVICE $SERVER_SERVICE_NAME MANUELLEMENT AVEC LA COMMANDE SUIVANTE:"
