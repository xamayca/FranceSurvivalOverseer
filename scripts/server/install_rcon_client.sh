...
install_rcon_cli(){
  ...
  rcon_permissions(){
    ...
    if chmod +x "$RCON_EXECUTABLE_PATH"; then
      ...
      log "[DEBUG] VEUILLEZ ESSAYER DE CONFIGURÉ LES PERMISSIONS MANUELLEMENT AVEC LA COMMANDE SUIVANTE:"
      log "[DEBUG] chmod +x $RCON_EXECUTABLE_PATH"
      exit 1
    fi
  }

  log "[LOG] VÉRIFICATION & INSTALLATION DE RCON CLI POUR LE SERVEUR ARK: $SERVER_SERVICE_NAME EN COURS..."
  if [[ -f "$RCON_EXECUTABLE_PATH" ]] && [[ -x "$RCON_EXECUTABLE_PATH" ]]; then
    ...
    log "[WARNING] RCON CLI N'EST PAS INSTALLÉ, POUR LE SERVEUR ARK: $SERVER_SERVICE_NAME, INSTALLATION EN COURS..."
    create_temp_directory
    download_rcon_cli
    verify_rcon_cli_integrity
    create_rcon_directory
