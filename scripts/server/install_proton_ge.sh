...
install_proton_ge(){
  ...
  if [[ -f "$PROTON_GE_EXECUTABLE_PATH" ]] && [[ -d "$STEAM_COMPAT_DATA_PATH/default_pfx" ]]; then
    log "[OK] PROTON GE $PROTON_GE_VERSION EST DÉJÀ INSTALLÉ SUR LE SERVEUR ARK: $SERVER_SERVICE_NAME."
  else
    create_proton_ge_temp_dir
    move_to_proton_ge_temp_dir
    download_proton_ge
    ...
    create_proton_ge_compat_data_folder
    copy_proton_ge_compat_data_files
    log "[SUCCESS] PROTON GE $PROTON_GE_VERSION EST INSTALLÉ AVEC SUCCÈS SUR LE SERVEUR ARK: $SERVER_SERVICE_NAME."
  fi
}
