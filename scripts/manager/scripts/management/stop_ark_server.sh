...
stop_ark_server() {
    log "[WARNING] ARRÊT DU SERVEUR ARK: $SERVER_SERVICE_NAME EN COURS..."
  ...
  log "[LOG] ARRÊT DU SERVICE $SERVER_SERVICE_NAME EN COURS..."
  if sudo systemctl stop "$SERVER_SERVICE_NAME"; then
    log "[SUCCESS] ARRÊT DU SERVICE $SERVER_SERVICE_NAME RÉUSSI."
  else
