...
auto_update_ark_server(){
  ...
  log "[WARNING] VÉRIFICATION DE LA MISE À JOUR DU SERVEUR ARK: $SERVER_SERVICE_NAME..."
  ...
  log "[LOG] VERIFICATION DU BUILD ID DU SERVEUR ARK: $SERVER_SERVICE_NAME DU FICHIER MANIFEST..."
  ...
  log "[LOG] VERIFICATION DU BUILD ID DU SERVEUR ARK: $SERVER_SERVICE_NAME DU SITE STEAM..."
  ...
  log "[LOG] COMPARAIOSN DES BUILD ID DU SERVEUR ARK: $SERVER_SERVICE_NAME ET DU SITE STEAM..."
  if [ "$server_build_id" -eq "$latest_build_id" ]; then
    log "[SUCCESS] LE SERVEUR ARK: $SERVER_SERVICE_NAME EST À JOUR."
    return 0
  else
    log "[WARNING] LE SERVEUR ARK: $SERVER_SERVICE_NAME N'EST PAS À JOUR."
    ...
    log "[LOG] REDÉMARRAGE DU SERVICE $SERVER_SERVICE_NAME POUR MISE À JOUR..."
